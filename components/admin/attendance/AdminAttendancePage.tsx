...
const AttendancePreviewDialog = dynamic(
  () => import('@/components/admin/attendance/AttendancePreviewDialog'),
  { ssr: false }
);
const AttendanceEditDialog = dynamic(
  () => import('@/components/admin/attendance/AttendanceEditDialog'),
  { ssr: false }
);
const AttendanceDeleteDialog = dynamic(
  () => import('@/components/admin/attendance/AttendanceDeleteDialog'),
  { ssr: false }
);
const AttendanceTimeEditDialog = dynamic(
  () =>
    import('@/components/admin/attendance/AttendanceTimeEditDialog').then(
      (m) => m.AttendanceTimeEditDialog
    ),
  { ssr: false }
);
...
export default function AdminAttendancePage() {
  ...
  function getAttendanceStatus(record?: AttendanceData): AttendanceStatusData | string {
    ...
    if (hasLate && hasEarlyLeave) return 'late_early_leave';
    if (hasLate) return 'late';
    if (hasEarlyLeave) return 'early_leave';
    return 'normal';
  }
        ...
        return (
          <Badge
            ...
            style={{
              ...
              ...(statusConfig.color === 'outline' && { borderColor: statusConfig.font_color }),
            }}
          >
    ...
    monthDays.forEach((date) => {
      ...
      if (existingRecords.length > 0) {
        ...
        const absentRecord: AttendanceData = {
          ...
          status: isWeekendDay ? 'normal' : 'absent',
