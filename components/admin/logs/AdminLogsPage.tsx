...
interface SystemLog {
  ...
  user_profiles?: { id: string; family_name: string; first_name: string };
}
...
export default function AdminLogsPage() {
  ...
  async function handleSystemLogsExport() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({ title: '成功', description: 'システムログをエクスポートしました' });
      } else {
        throw new Error('Export failed');
      }
    } catch (error) {
      toast({ title: 'エラー', description: 'エクスポートに失敗しました', variant: 'destructive' });
    } finally {
  ...
  async function handleAuditLogsExport() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({ title: '成功', description: '監査ログをエクスポートしました' });
      } else {
        throw new Error('Export failed');
      }
    } catch (error) {
  ...
  function handleColumnSettingsApply() {
    toast({ title: '設定を適用しました', description: '表示項目の設定が更新されました' });
  }
  ...
  function handleColumnSettingsReset() {
    ...
    toast({ title: '初期設定に戻しました', description: '表示項目を初期設定に戻しました' });
  }
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-900 break-all">{log.user_agent || '-'}</p>
          </div>
        </div>
                ...
                ? [
                    ...
                    <div key={key} className="flex items-start space-x-2 p-2 rounded border">
                      <input
                        ...
                        onChange={(e) =>
                          setSystemLogColumns((prev) => ({ ...prev, [key]: e.target.checked }))
                        }
                ...
                : [
                    ...
                    <div key={key} className="flex items-start space-x-2 p-2 rounded border">
                      <input
                        ...
                        onChange={(e) =>
                          setAuditLogColumns((prev) => ({ ...prev, [key]: e.target.checked }))
                        }
                        className="mt-1 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                      />
                      <div className="flex-1">
