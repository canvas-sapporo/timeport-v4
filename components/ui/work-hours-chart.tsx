...
export default function WorkHoursChart({
  ...
  if (data.length > 0) {
    ...
    console.log('TODO data in sample:', {
      completedTodos: data[0].completedTodos,
      pendingTodos: data[0].pendingTodos,
    });

    // TODOデータの統計
    const todoStats = data.reduce(
      (acc, item) => {
        acc.completed += item.completedTodos;
        acc.pending += item.pendingTodos;
        return acc;
      },
      { completed: 0, pending: 0 }
    );
    console.log('Total TODO stats:', todoStats);
  }
  ...
  const formatTooltip = (value: number, name: string) => {
    ...
    if (name === 'completedTodos') return [`${value}件`, '完了タスク'];
    if (name === 'pendingTodos') return [`${value}件`, '未完了タスク'];
    return [value, name];
  };
